...
with t7:
    ...
    def dibuixa_mapa_tir_fcbq(tirs_list, titol="Mapa de tir", W=560, H=300):
        svg = []
        svg.append(f'<svg viewBox="0 0 {W} {H+40}" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:700px;border-radius:10px">')
        svg.append(f'<rect width="{W}" height="{H}" fill="#e8f0e8" rx="6"/>')
        s = 'stroke="#374151" stroke-width="1.5" fill="none"'
        svg.append(f'<rect x="8" y="8" width="{W-16}" height="{H-16}" {s} rx="3"/>')
        svg.append(f'<line x1="{W//2}" y1="8" x2="{W//2}" y2="{H-8}" {s}/>')
        svg.append(f'<circle cx="{W//2}" cy="{H//2}" r="32" {s}/>')
        svg.append(f'<rect x="8" y="{H//2-42}" width="100" height="84" {s}/>')
        svg.append(f'<rect x="{W-108}" y="{H//2-42}" width="100" height="84" {s}/>')
        svg.append(f'<path d="M 8 {H//2-95} A 138 138 0 0 1 8 {H//2+95}" {s}/>')
        svg.append(f'<path d="M {W-8} {H//2-95} A 138 138 0 0 0 {W-8} {H//2+95}" {s}/>')
        svg.append(f'<circle cx="20" cy="{H//2}" r="4" fill="#374151"/>')
        svg.append(f'<circle cx="{W-20}" cy="{H//2}" r="4" fill="#374151"/>')
        ...
        for t in tirs_list:
            ...
            if fet:
                fets_n += 1
                svg.append(f'<circle cx="{cx:.1f}" cy="{cy:.1f}" r="6" fill="#16a34a" opacity="0.85" stroke="white" stroke-width="1.5"><title>✅ Cistella</title></circle>')
            else:
                fallats_n += 1
                svg.append(f'<line x1="{cx-4:.1f}" y1="{cy-4:.1f}" x2="{cx+4:.1f}" y2="{cy+4:.1f}" stroke="#dc2626" stroke-width="2.5" opacity="0.75"/>')
                svg.append(f'<line x1="{cx+4:.1f}" y1="{cy-4:.1f}" x2="{cx-4:.1f}" y2="{cy+4:.1f}" stroke="#dc2626" stroke-width="2.5" opacity="0.75"/>')
        ef = round(fets_n/max(fets_n+fallats_n,1)*100)
        svg.append(f'<text x="{W//2}" y="{H+18}" font-family="Arial" font-size="11" fill="#374151" text-anchor="middle" font-weight="bold">{titol}</text>')
        svg.append(f'<circle cx="15" cy="{H+32}" r="5" fill="#16a34a"/>')
        svg.append(f'<text x="25" y="{H+36}" font-family="Arial" font-size="10" fill="#374151">✅ {fets_n} cistelles  ❌ {fallats_n} fallats  ·  {ef}% ef.</text>')
        svg.append('</svg>')
        return ''.join(svg)
