...
with t5:
    ...
    # ── Eficiència ofensiva/defensiva i On/Off Rating ───────────────────────
    ...
    if df_pr_onoff.empty:
        ...
        df_onoff = load_jugades_db(sel_onoff)
        if not df_onoff.empty:
            teams_oo = get_teams_ordered(df_onoff)
            p_row = df_pr_onoff[df_pr_onoff["match_id"]==sel_onoff].iloc[0]
            tn_oo = {}
            if len(teams_oo)>=1: tn_oo[teams_oo[0]] = p_row["nom_a"]
            if len(teams_oo)>=2: tn_oo[teams_oo[1]] = p_row["nom_b"]
            df_onoff["equip_nom"] = df_onoff["idEquip"].map(tn_oo).fillna("?")
            df_onoff["jugador"] = df_onoff.get("jugadora", df_onoff.get("jugador", ""))

            # Eficiències d'equip
            st.markdown("**Eficiències d'equip**")
            ef = calc_eficiencies(df_onoff, teams_oo, tn_oo)
            col_ea, col_eb = st.columns(2)
            for col_e, tid in [(col_ea, teams_oo[0] if teams_oo else None),
                               (col_eb, teams_oo[1] if len(teams_oo)>1 else None)]:
                with col_e:
                    if tid and tid in ef:
                        e = ef[tid]
                        tcol = COLOR_A if tid == teams_oo[0] else COLOR_B
                        c1,c2,c3,c4 = st.columns(4)
                        with c1: st.markdown(card("Possessions", e["poss_of"], "", tcol), unsafe_allow_html=True)
                        with c2: st.markdown(card("Off Rating", e["off_rtg"], "pts/100 poss", tcol), unsafe_allow_html=True)
                        with c3: st.markdown(card("Def Rating", e["def_rtg"], "pts/100 poss", "#dc2626"), unsafe_allow_html=True)
                        nc = "#16a34a" if e["net_rtg"] >= 0 else "#dc2626"
                        with c4: st.markdown(card("Net Rating",
                            f"{'+'if e['net_rtg']>=0 else ''}{e['net_rtg']}",
                            e["nom"], nc), unsafe_allow_html=True)

            # On/Off per jugadora
            st.markdown("**On/Off Rating per jugadora**")
            eq_oo = st.selectbox("Equip", [tn_oo.get(t,"?") for t in teams_oo], key="eq_oo")
            tid_oo = teams_oo[0] if eq_oo == tn_oo.get(teams_oo[0],"?") else (teams_oo[1] if len(teams_oo)>1 else None)

            if tid_oo:
                col_jug = "jugadora" if "jugadora" in df_onoff.columns else "jugador"
                jugs_oo = sorted(df_onoff[
                    (df_onoff["idEquip"]==tid_oo) & (df_onoff[col_jug] != "")
                ][col_jug].unique().tolist())

                oo_rows = []
                for jug in jugs_oo:
                    df_jug_oo = df_onoff.copy()
                    df_jug_oo["jugador"] = df_jug_oo[col_jug]
                    oo = calc_onoff(df_jug_oo, jug, tid_oo, teams_oo)
                    if oo:
                        oo_rows.append({
                            "Jugadora": jug,
                            "On Off Rtg": oo["on_off_rtg"],
                            "On Def Rtg": oo["on_def_rtg"],
                            "On Net Rtg": oo["on_net_rtg"],
                            "Off Off Rtg": oo["off_off_rtg"],
                            "Off Def Rtg": oo["off_def_rtg"],
                            "Off Net Rtg": oo["off_net_rtg"],
                            "Diferència": f"{'+'if oo['diff']>=0 else ''}{oo['diff']}",
                            "_diff": oo["diff"]
                        })

                if oo_rows:
                    df_oo = pd.DataFrame(oo_rows).sort_values("_diff", ascending=False)
                    colors_bar = ["#16a34a" if d >= 0 else "#dc2626" for d in df_oo["_diff"]]
                    fig_oo = go.Figure()
                    fig_oo.add_trace(go.Bar(
                        x=df_oo["Jugadora"], y=df_oo["_diff"],
                        marker_color=colors_bar,
                        text=[f"{'+'if d>=0 else ''}{d}" for d in df_oo["_diff"]],
                        textposition="outside", name="On-Off Net Rating"))
                    fig_oo.add_hline(y=0, line_dash="solid", line_color="#e2e4e8")
                    ...
                    with st.expander("Veure detall complet"):
                        ...
                        st.dataframe(df_oo.drop(columns="_diff"),
                            use_container_width=True, hide_index=True)
                else:
