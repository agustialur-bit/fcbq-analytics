...
with t7:
    ...
    def dibuixa_mapa_tir_fcbq(tirs_list, titol="Mapa de tir", W=400, H=340):
        """Dibuixa mig camp de bàsquet. Les coordenades venen del web FCBQ
        on cada meitat de pantalla és un mig camp amb la cistella a dalt al centre."""
        svg = []
        svg.append(f'<svg viewBox="0 0 {W} {H+35}" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:500px;border-radius:10px">')
        # Fons verd clar (pista)
        svg.append(f'<rect width="{W}" height="{H}" fill="#c8dfc8" rx="6"/>')
        s = 'stroke="#2d5a2d" stroke-width="1.5" fill="none"'
        sf = 'stroke="#2d5a2d" stroke-width="1.5" fill="rgba(255,255,255,0.15)"'
        # Perímetre
        svg.append(f'<rect x="6" y="6" width="{W-12}" height="{H-12}" {s} rx="3"/>')
        # Cistella (dalt al centre)
        cx_cist = W//2; cy_cist = 28
        svg.append(f'<circle cx="{cx_cist}" cy="{cy_cist}" r="8" {sf}/>')
        svg.append(f'<circle cx="{cx_cist}" cy="{cy_cist}" r="3" fill="#2d5a2d"/>')
        # Taula (rectangle sota la cistella)
        svg.append(f'<rect x="{cx_cist-30}" y="6" width="60" height="{int(H*0.28)}" {sf}/>')
        # Zona pintada (rectangle gran)
        zona_w = int(W*0.42); zona_h = int(H*0.38)
        svg.append(f'<rect x="{(W-zona_w)//2}" y="6" width="{zona_w}" height="{zona_h}" {sf}/>')
        # Arc de zona (cercle sota la zona pintada)
        svg.append(f'<circle cx="{W//2}" cy="{6+zona_h}" r="{zona_w//4}" {s}/>')
        # Arc de triple
        arc_r = int(W*0.56)
        svg.append(f'<path d="M 6 {int(H*0.55)} A {arc_r} {arc_r} 0 0 1 {W-6} {int(H*0.55)}" {s}/>')
        # Línies laterals de triple
        svg.append(f'<line x1="6" y1="6" x2="6" y2="{int(H*0.55)}" {s}/>')
        svg.append(f'<line x1="{W-6}" y1="6" x2="{W-6}" y2="{int(H*0.55)}" {s}/>')
        # Línia de mig camp (baix)
        svg.append(f'<line x1="6" y1="{H-6}" x2="{W-6}" y2="{H-6}" {s}/>')

        # Tirs — les coordenades del web estan sobre el mig camp
        # x: 0-100% de l'ample del mig camp, y: 0-100% de l'alçada
        fets_n = 0; fallats_n = 0
        for t in tirs_list:
            # Normalitza x al rang real (el web pot usar 0-50 o 50-100)
            x_raw = float(t['x'])
            # Si és equip visitant (x > 50), normalitza a 0-100
            if x_raw > 50:
                x_norm = (x_raw - 50) * 2
            else:
                x_norm = x_raw * 2
            ...
            if fet:
                fets_n += 1
                svg.append(f'<circle cx="{px:.1f}" cy="{py:.1f}" r="7" fill="#16a34a" opacity="0.85" stroke="white" stroke-width="1.5"><title>✅ Cistella</title></circle>')
            else:
                fallats_n += 1
                svg.append(f'<line x1="{px-5:.1f}" y1="{py-5:.1f}" x2="{px+5:.1f}" y2="{py+5:.1f}" stroke="#dc2626" stroke-width="2.5" opacity="0.8"/>')
                svg.append(f'<line x1="{px+5:.1f}" y1="{py-5:.1f}" x2="{px-5:.1f}" y2="{py+5:.1f}" stroke="#dc2626" stroke-width="2.5" opacity="0.8"/>')

        ef = round(fets_n/max(fets_n+fallats_n,1)*100)
        svg.append(f'<text x="{W//2}" y="{H+16}" font-family="Arial" font-size="11" fill="#374151" text-anchor="middle" font-weight="bold">{titol}</text>')
        svg.append(f'<text x="{W//2}" y="{H+30}" font-family="Arial" font-size="10" fill="#374151" text-anchor="middle">✅ {fets_n} cistelles  ❌ {fallats_n} fallats  ·  {ef}% ef.</text>')
        svg.append('</svg>')
        return ''.join(svg)
...
console.log(`✅ Copiat! Total: ${punts.length} | Cistelles: ${punts.filter(p=>p.fet).length} | Fallats: ${punts.filter(p=>!p.fet).length}`);""", language="javascript")
    ...
    if json_tirs.strip():
        try:
            ...
            st.markdown(dibuixa_mapa_tir_fcbq(tirs_show, f"{eq_sel}"), unsafe_allow_html=True)
    ...
    if df_tirs_bd.empty:
        ...
        if sel_partits:
            ...
            st.markdown(dibuixa_mapa_tir_fcbq(tirs_acum, f"{eq_acum} — {len(sel_partits)} partits"), unsafe_allow_html=True)
