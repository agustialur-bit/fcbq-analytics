...
def save_stats_jugador(match_id, data_consulta, df, teams, team_names):
    ...
    for jug in df["jugador"].unique():
        ...
        for _, row_imp in df.sort_values("num").iterrows():
            jug_imp = str(row_imp.get("jugador", row_imp.get("jugadora","")))
            ...
            if tid_oo2:
                ...
                for jug2 in jugs_oo2:
                    ...
                    if oo2:
                        oo_rows2.append({
                            "Jugadora": jug2,
                            "On Off Rtg": oo2["on_off_rtg"],
                            "On Def Rtg": oo2["on_def_rtg"],
                            "On Net Rtg": oo2["on_net_rtg"],
                            "Off Off Rtg": oo2["off_off_rtg"],
                            "Off Def Rtg": oo2["off_def_rtg"],
                            "Off Net Rtg": oo2["off_net_rtg"],
...
with t5:
    ...
    if tid_rot and intervals_jug:
        ...
        if len(jugs_eq_all) >= 2:
            ...
            noms_curts = [n.split()[-1] if " " in n else n for n in jugs_eq_all]
