...
with t7:
    ...
    def dibuixa_mapa_tir_fcbq(tirs_list, titol="Mapa de tir", equip_side="local", W=340, H=400):
        ...
        # ── Tirs ──────────────────────────────────────────────────────────
        # La pista FCBQ és horitzontal. Rotem 90°:
        # LOCAL: cistella a x=50 (dreta) → rotació: px=y*W, py=(1-x/50)*H → x=50→py=0 (dalt) ✓
        # VISITANT: cistella a x=50 (esquerra) → rotació: px=y*W, py=((x-50)/50)*H → x=50→py=0 ✓
        ...
        for t in tirs_list:
            ...
            if equip_side == "local":
                # x_raw: 0-50, cistella a x=50
                # y_raw: 0-100, cistella a y=50 (centre vertical FCBQ)
                px = (y_raw / 100) * W
                py = (1 - x_raw / 50) * H
            else:
                # x_raw: 50-100, cistella a x=50
                px = (y_raw / 100) * W
                py = ((x_raw - 50) / 50) * H
